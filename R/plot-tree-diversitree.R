## This code is directly from diversitree's plot2.phylo
...
pp.segments.fan <- function(phy, xy.seg, col, lwd, lty, np=1000) {
  ...
  if ( any(!is.na(i)) ) # For a two branch tree.
    with(xy.seg2[[1]],
         diversitree:::arcs(theta0, theta1, r1, col[i], lty[i], lwd[i], np))
}  
...
pp.clades.fan <- function(phy, xy, xy.seg, border, fill, lwd) {
  ...
  diversitree:::sectors(t0, t1, r0, r1, border=border[i], col=fill[i], lwd=lwd)
}
...
pp.tiplabel.fan <- function(phy, xy, label.offset, adj, cex,
  ...
  diversitree:::radial.text(r, theta, phy$tip.label, cex=cex, col=col, font=font)
}
